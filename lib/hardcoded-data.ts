export const specialties = [
  { id: 1, name: "Nội Tổng quát", price: 250000 },
  { id: 2, name: "Nhi khoa", price: 300000 },
  { id: 3, name: "Tim mạch", price: 350000 },
  { id: 4, name: "Thần kinh", price: 400000 },
  { id: 5, name: "Hô hấp", price: 300000 },
  { id: 6, name: "Da liễu", price: 250000 },
  { id: 7, name: "Tiêu hóa", price: 350000 },
  { id: 8, name: "Cơ xương khớp", price: 300000 },
  { id: 9, name: "Tai Mũi Họng", price: 250000 },
  { id: 10, name: "Mắt", price: 250000 },
  { id: 11, name: "Sản phụ khoa", price: 350000 },
  { id: 12, name: "Nam khoa", price: 300000 },
  { id: 13, name: "Nội tiết", price: 350000 },
  { id: 14, name: "Thận - Tiết niệu", price: 300000 },
  { id: 15, name: "Ung bướu", price: 400000 },
  { id: 16, name: "Tâm thần", price: 350000 },
  { id: 17, name: "Dinh dưỡng", price: 250000 },
  { id: 18, name: "Huyết học", price: 350000 },
  { id: 19, name: "Ngoại Tổng quát", price: 300000 },
  { id: 20, name: "Chấn thương chỉnh hình", price: 350000 },
  { id: 21, name: "Phẫu thuật thẩm mỹ", price: 500000 },
  { id: 22, name: "Nha khoa", price: 300000 },
  { id: 23, name: "Vật lý trị liệu", price: 250000 },
  { id: 24, name: "Y học cổ truyền", price: 200000 },
  { id: 25, name: "Dị ứng - Miễn dịch", price: 350000 },
  { id: 26, name: "Truyền nhiễm", price: 300000 },
  { id: 27, name: "Phục hồi chức năng", price: 250000 },
  { id: 28, name: "Tạo hình thẩm mỹ", price: 450000 },
  { id: 29, name: "Lão khoa", price: 300000 },
  { id: 30, name: "Giải phẫu bệnh", price: 400000 },
];
export const medicationData = [
  {
    medicationName: "Paracetamol",
    dosage: "500mg",
    quantity: 20,
    instructions: "Uống 1 viên mỗi 6 giờ khi cần.",
    price: 2000,
  },
  {
    medicationName: "Ibuprofen",
    dosage: "200mg",
    quantity: 30,
    instructions: "Uống 1-2 viên mỗi 8 giờ.",
    price: 3000,
  },
  {
    medicationName: "Amoxicillin",
    dosage: "500mg",
    quantity: 14,
    instructions: "Uống 1 viên mỗi 12 giờ trong 7 ngày.",
    price: 5000,
  },
  {
    medicationName: "Loratadine",
    dosage: "10mg",
    quantity: 10,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 4000,
  },
  {
    medicationName: "Metformin",
    dosage: "500mg",
    quantity: 60,
    instructions: "Uống 1 viên mỗi bữa ăn.",
    price: 1500,
  },
  {
    medicationName: "Simvastatin",
    dosage: "20mg",
    quantity: 30,
    instructions: "Uống 1 viên trước khi đi ngủ.",
    price: 7000,
  },
  {
    medicationName: "Omeprazole",
    dosage: "20mg",
    quantity: 28,
    instructions: "Uống 1 viên mỗi sáng trước khi ăn.",
    price: 6000,
  },
  {
    medicationName: "Aspirin",
    dosage: "81mg",
    quantity: 100,
    instructions: "Uống 1 viên mỗi ngày sau bữa ăn.",
    price: 1000,
  },
  {
    medicationName: "Cetrizine",
    dosage: "10mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 5000,
  },
  {
    medicationName: "Clopidogrel",
    dosage: "75mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 6000,
  },
  {
    medicationName: "Levothyroxine",
    dosage: "50mcg",
    quantity: 30,
    instructions: "Uống 1 viên vào buổi sáng khi bụng đói.",
    price: 2500,
  },
  {
    medicationName: "Furosemide",
    dosage: "40mg",
    quantity: 20,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 2000,
  },
  {
    medicationName: "Doxycycline",
    dosage: "100mg",
    quantity: 14,
    instructions: "Uống 1 viên mỗi 12 giờ.",
    price: 4500,
  },
  {
    medicationName: "Azithromycin",
    dosage: "250mg",
    quantity: 6,
    instructions: "Uống 2 viên ngày đầu, sau đó 1 viên mỗi ngày.",
    price: 12000,
  },
  {
    medicationName: "Prednisone",
    dosage: "10mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày hoặc theo chỉ định.",
    price: 3000,
  },
  {
    medicationName: "Hydrochlorothiazide",
    dosage: "25mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi sáng.",
    price: 2000,
  },
  {
    medicationName: "Warfarin",
    dosage: "5mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 5000,
  },
  {
    medicationName: "Alprazolam",
    dosage: "0.5mg",
    quantity: 14,
    instructions: "Uống 1 viên mỗi 8 giờ khi cần.",
    price: 8000,
  },
  {
    medicationName: "Ciprofloxacin",
    dosage: "500mg",
    quantity: 10,
    instructions: "Uống 1 viên mỗi 12 giờ.",
    price: 7000,
  },
  {
    medicationName: "Metronidazole",
    dosage: "500mg",
    quantity: 20,
    instructions: "Uống 1 viên mỗi 8 giờ.",
    price: 4000,
  },
  {
    medicationName: "Lisinopril",
    dosage: "10mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 5000,
  },
  {
    medicationName: "Gabapentin",
    dosage: "300mg",
    quantity: 90,
    instructions: "Uống 1 viên mỗi 8 giờ.",
    price: 10000,
  },
  {
    medicationName: "Meloxicam",
    dosage: "15mg",
    quantity: 10,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 3500,
  },
  {
    medicationName: "Allopurinol",
    dosage: "300mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 4500,
  },
  {
    medicationName: "Atorvastatin",
    dosage: "20mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi tối.",
    price: 7000,
  },
  {
    medicationName: "Esomeprazole",
    dosage: "40mg",
    quantity: 28,
    instructions: "Uống 1 viên mỗi ngày trước khi ăn.",
    price: 9500,
  },
  {
    medicationName: "Tamsulosin",
    dosage: "0.4mg",
    quantity: 30,
    instructions: "Uống 1 viên sau bữa ăn tối.",
    price: 6000,
  },
  {
    medicationName: "Amlodipine",
    dosage: "5mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 5000,
  },
  {
    medicationName: "Venlafaxine",
    dosage: "75mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 8500,
  },
  {
    medicationName: "Clonazepam",
    dosage: "0.5mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 7500,
  },
  {
    medicationName: "Sertraline",
    dosage: "50mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 8000,
  },
  {
    medicationName: "Zolpidem",
    dosage: "10mg",
    quantity: 10,
    instructions: "Uống 1 viên trước khi đi ngủ.",
    price: 12000,
  },
  {
    medicationName: "Tramadol",
    dosage: "50mg",
    quantity: 20,
    instructions: "Uống 1 viên mỗi 6 giờ khi cần.",
    price: 4000,
  },
  {
    medicationName: "Fluoxetine",
    dosage: "20mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 7500,
  },
  {
    medicationName: "Bupropion",
    dosage: "150mg",
    quantity: 60,
    instructions: "Uống 1 viên mỗi 12 giờ.",
    price: 15000,
  },
  {
    medicationName: "Ranitidine",
    dosage: "150mg",
    quantity: 20,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 3500,
  },
  {
    medicationName: "Montelukast",
    dosage: "10mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày vào buổi tối.",
    price: 8500,
  },
  {
    medicationName: "Duloxetine",
    dosage: "60mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày.",
    price: 9500,
  },
  {
    medicationName: "Mirtazapine",
    dosage: "15mg",
    quantity: 30,
    instructions: "Uống 1 viên trước khi đi ngủ.",
    price: 7000,
  },
  {
    medicationName: "Famotidine",
    dosage: "20mg",
    quantity: 30,
    instructions: "Uống 1 viên mỗi ngày trước khi ăn.",
    price: 5000,
  },
];

export const labTestsData = [
  {
    _id: 1,
    testName: "Xét nghiệm máu tổng quát",
    description: "Kiểm tra các chỉ số cơ bản trong máu.",
    price: 150000,
  },
  {
    _id: 2,
    testName: "Xét nghiệm đường huyết",
    description: "Đo lượng đường trong máu.",
    price: 160000,
  },
  {
    _id: 3,
    testName: "Xét nghiệm cholesterol",
    description: "Đánh giá mức độ cholesterol trong máu.",
    price: 140000,
  },
  {
    _id: 4,
    testName: "Xét nghiệm chức năng gan",
    description: "Kiểm tra sức khỏe của gan.",
    price: 210000,
  },
  {
    _id: 5,
    testName: "Xét nghiệm chức năng thận",
    description: "Đánh giá chức năng thận.",
    price: 140000,
  },
  {
    _id: 6,
    testName: "Xét nghiệm viêm gan B",
    description: "Kiểm tra sự hiện diện của virus viêm gan B.",
    price: 300000,
  },
  {
    _id: 7,
    testName: "Xét nghiệm viêm gan C",
    description: "Kiểm tra sự hiện diện của virus viêm gan C.",
    price: 140000,
  },
  {
    _id: 8,
    testName: "Xét nghiệm HIV",
    description: "Xét nghiệm virus HIV.",
    price: 140000,
  },
  {
    _id: 9,
    testName: "Xét nghiệm hormon tuyến giáp",
    description: "Kiểm tra các chỉ số hormon tuyến giáp.",
    price: 140000,
  },
  {
    _id: 10,
    testName: "Xét nghiệm nước tiểu",
    description: "Phân tích mẫu nước tiểu.",
    price: 140000,
  },
  {
    _id: 11,
    testName: "Xét nghiệm điện giải đồ",
    description: "Đo các điện giải trong máu.",
    price: 100000,
  },
  {
    _id: 12,
    testName: "Xét nghiệm nhóm máu",
    description: "Xác định nhóm máu của bệnh nhân.",
    price: 100000,
  },
  {
    _id: 13,
    testName: "Xét nghiệm kháng thể",
    description: "Kiểm tra các kháng thể trong cơ thể.",
    price: 100000,
  },
  {
    _id: 14,
    testName: "Xét nghiệm mỡ máu",
    description: "Đo mức triglycer_ides trong máu.",
    price: 100000,
  },
  {
    _id: 15,
    testName: "Xét nghiệm vi sinh",
    description: "Nuôi cấy vi sinh vật từ mẫu bệnh phẩm.",
    price: 100000,
  },
  {
    _id: 16,
    testName: "Xét nghiệm phản ứng viêm",
    description: "Đánh giá mức độ viêm trong cơ thể.",
    price: 200000,
  },
  {
    _id: 17,
    testName: "Xét nghiệm tinh dịch",
    description: "Phân tích mẫu tinh dịch.",
    price: 200000,
  },
  {
    _id: 18,
    testName: "Xét nghiệm dị ứng",
    description: "Kiểm tra phản ứng dị ứng.",
    price: 200000,
  },
  {
    _id: 19,
    testName: "Xét nghiệm ung thư",
    description: "Xét nghiệm các chỉ số liên quan đến ung thư.",
    price: 200000,
  },
  {
    _id: 20,
    testName: "Xét nghiệm COVID-19",
    description: "Kiểm tra sự hiện diện của virus COVID-19.",
    price: 200000,
  },
];

export const processSteps = [
  {
    title: "LÊN LỊCH HẸN KHÁM",
    details: [
      "Đăng ký sử dụng hệ thống đặt lịch hẹn trực tuyến.",
      "Chọn hình thức khám: Theo ngày hoặc theo khung giờ.",
      "Nhập thông tin bệnh nhân: Số hồ sơ bệnh án hoặc bệnh nhân mới.",
      "Chọn thông tin khám: Chuyên khoa, bác sĩ, ngày khám, giờ khám (nếu có).",
    ],
  },
  {
    title: "THANH TOÁN PHÍ KHÁM",
    details: [
      "Chọn phương thức thanh toán: Thẻ bệnh viện, căn cước công dân, hoặc thẻ ATM, v.v.",
      "Kiểm tra số tiền cần thanh toán, chọn phương thức thanh toán.",
      "Nhập thông tin để hoàn tất thanh toán.",
    ],
  },
  {
    title: "XÁC NHẬN BỆNH NHÂN ĐẾN BỆNH VIỆN",
    details: [
      "Sau khi thanh toán thành công, xác nhận sẽ được gửi qua email, tin nhắn hoặc trên trang web.",
      "Vào ngày khám:",
      "Bệnh nhân không có hồ sơ bệnh án đến quầy tiếp nhận trước giờ hẹn 15-30 phút.",
      "Bệnh nhân có hồ sơ bệnh án đến trước giờ hẹn 15-30 phút để xác nhận thông tin trước khi vào phòng khám.",
    ],
  },
  {
    title: "KHÁM VÀ XÉT NGHIỆM",
    details: [
      "Bệnh nhân được khám tại phòng khám chuyên khoa theo lịch hẹn.",
      "Thực hiện các xét nghiệm và chẩn đoán hình ảnh (nếu có) theo chỉ định của bác sĩ.",
      "Sau khi nhận kết quả xét nghiệm, bệnh nhân quay lại phòng khám để bác sĩ đánh giá và kê đơn điều trị.",
    ],
  },
  {
    title: "NHẬN THUỐC",
    details: [
      "Bệnh nhân có bảo hiểm y tế: Nhận thuốc tại quầy 17, 18, 19, 20 sau khi thanh toán phần chênh lệch (nếu có).",
      "Bệnh nhân không có bảo hiểm y tế: Nhận thuốc tại quầy A hoặc B sau khi thanh toán tại quầy thu ngân hoặc chuyển khoản vào tài khoản thanh toán khám (nếu có).",
    ],
  },
  {
    title: "LÊN LỊCH TÁI KHÁM",
    details: [
      "Sử dụng hệ thống đặt lịch hẹn như ở BƯỚC 1 và BƯỚC 2 để nhận phiếu hẹn điện tử.",
    ],
  },
];

export const reqTestData = [
  {
    _id: "YCXN-033564",
    test: [
      {
        _id: "1",
        testName: "Xét nghiệm viêm gan B",
        price: 300000,
        description: "",
      },
      {
        _id: "2",
        testName: "Xét nghiệm chức năng thận",
        price: 140000,
        description: "",
      },
    ],
    patientId: "BN-5C662W",
    doctorId: "BS-98ANP7",
    reason: "Xin lỗi anh chịu hông nổi, xin lỗi vì quá đẹp trai",
    requestDate: new Date(),
  },
  {
    _id: "YCXN-033546",
    test: [
      {
        _id: "3",
        testName: "Xét nghiệm dị ứng",
        price: 200000,
        description: "",
      },
      {
        _id: "4",
        testName: "Xét nghiệm ung thư",
        price: 200000,
        description: "",
      },
    ],
    patientId: "BN-PMQ7TS",
    doctorId: "BS-98ANP7",
    reason:
      "Nhận ra mình ở quận ba đình, quay đầu thấy mấy thèn em ở trên cầu giấy",
    requestDate: new Date(),
  },
];

export const doctorsData = [
  {
    _id: "BS-98ANP7",
    role: "doctor",
    email: "duclam24@gmail.com",
    password: "$2a$12$NXY3LuE4x0fD1qRMDEILkuOB6VSt7VwLwh33wMVSxGz.oeXiyJ14C",
    isOnline: true,
    roomNumber: "000",
    schedule: [
      {
        dayOfWeek: "Monday",
        startTime: "09:00",
        endTime: "17:00",
        _id: { $oid: "6710e31a19d4c9bd1bd8fd94" },
      },
      {
        dayOfWeek: "Tuesday",
        startTime: "09:00",
        endTime: "16:00",
        _id: { $oid: "6710e31a19d4c9bd1bd8fd95" },
      },
      {
        dayOfWeek: "Wednesday",
        startTime: "09:00",
        endTime: "15:00",
        _id: { $oid: "6710e31a19d4c9bd1bd8fd96" },
      },
      {
        dayOfWeek: "Friday",
        startTime: "09:00",
        endTime: "15:00",
        _id: { $oid: "6710e31a19d4c9bd1bd8fd98" },
      },
    ],
    appointmentList: [],
    createdAt: { $date: { $numberLong: "1729159398965" } },
    updatedAt: { $date: { $numberLong: "1730483433861" } },
    __v: { $numberInt: "0" },
    fullName: "Hồ Đức Lâm",
    gender: "Male",
    phone: "3529234422",
    specialization: "Cardiology",
  },
  {
    _id: "BS-SS1CRG",
    fullName: "Trần Lâm",
    role: "doctor",
    gender: "Male",
    phone: "84352923442",
    email: "doctor2408@gmail.com",
    password: "$2a$12$CUwNvi0RXl9zrehzlIu21OHNen1eNUbcLPrtv0fKzf0xicQBzPBhK",
    isOnline: true,
    roomNumber: "001",
    schedule: [
      {
        dayOfWeek: "Monday",
        startTime: "09:00",
        endTime: "17:00",
        _id: { $oid: "6716554b876ce18d41a0bf30" },
      },
      {
        dayOfWeek: "Tuesday",
        startTime: "09:00",
        endTime: "17:00",
        _id: { $oid: "6716554b876ce18d41a0bf31" },
      },
      {
        dayOfWeek: "Wednesday",
        startTime: "09:00",
        endTime: "17:00",
        _id: { $oid: "6716554b876ce18d41a0bf32" },
      },
      {
        dayOfWeek: "Thursday",
        startTime: "09:00",
        endTime: "17:00",
        _id: { $oid: "6716554b876ce18d41a0bf33" },
      },
      {
        dayOfWeek: "Saturday",
        startTime: "09:00",
        endTime: "12:00",
        _id: { $oid: "6716554b876ce18d41a0bf35" },
      },
    ],
    appointmentList: [],
    createdAt: { $date: { $numberLong: "1729516220538" } },
    updatedAt: { $date: { $numberLong: "1730483709154" } },
    __v: { $numberInt: "0" },
    specialization: "Cardiology",
  },
];

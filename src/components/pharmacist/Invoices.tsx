...
export default function Invoices() {
  ...
  const fetchData = async () => {
    try {
      ...
      const presInvoices = response.data.filter((item: any) => {
        return item.type === "presInvoice";
      });
      setInvoices(presInvoices);
      setIsLoading(false);
    } catch (error) {
      console.error(error);
    }
  ...
  return (
    <div className="w-full flex flex-col gap-4 bg-background border rounded-md p-4 h-[100%] overflow-auto">
      <p className="text-base font-semibold text-blue-500">
        DANH SÁCH CÁC HOÁ ĐƠN ĐƠN THUỐC ĐÃ LẬP
      </p>
      <div className="flex flex-row gap-3">
        ...
        <Select value={filterRole} onValueChange={setFilterRole}>
          ...
          <SelectContent>
            ...
            <SelectItem value="medicalInvoice">Hôm nay</SelectItem>
            <SelectItem value="medicalInvoice">Mới nhất</SelectItem>
            <SelectItem value="testInvoice">Cũ nhất</SelectItem>
          </SelectContent>
        </Select>
        <Button
          variant="outline"
